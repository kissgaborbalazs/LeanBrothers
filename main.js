...
const SECTION_IDS = [
  ...
  'miert-mi','rolunk','partner','gyik','cta','kapcsolat'
];
...
function buildFaqItems() {
  ...
  // Fallback: DOM-ból olvassa ki a data-hu / data-en (kérdés) és
  // data-hu-a / data-en-a (válasz) attribútumokat
  ...
  faqItems = Array.from(document.querySelectorAll('#faqGrid .faq-q')).map(btn => ({
    ...
    a: btn.getAttribute('data-' + lang + '-a') || ''
  }));
}
...
async function loadLang(lang) {
  try {
    ...
    // Fontos: itt is frissíteni kell a currentLang-ot és a localStorage-ot,
    // különben az updateLangUI() a régi nyelv szerint jelöl (HU marad aktív).
...
function applyTranslations() {
  ...
  const sectionLabels = {
    hu: ['Hero','A probléma','Módszerünk','Szolgáltatások','Miért mi','Csapat','Háttér','GYIK','CTA','Kapcsolat'],
    en: ['Hero','The Problem','Our Method','Services','Why Us','Team','Backing','FAQ','CTA','Contact']
  };
